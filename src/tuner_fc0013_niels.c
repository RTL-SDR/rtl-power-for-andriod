...
// Enable/disable VHF filter, and cache the current setting
static int vhf_enabled = -1;
static int fc0013_enable_vhf_filter(void *dev, int enable)
{
	int ret = 0;
	uint8_t tmp;

	// Avoid extra changes
	if (vhf_enabled==enable)
		return ret;
...
static int uhf_mode = -1;
static int fc0013_set_uhf_mode(void *dev, int mode)
{
	int ret = 0;
	uint8_t tmp;
...
int fc0013_set_params(void *dev, uint32_t freq, uint32_t bandwidth)
{
	...
	if (freq < 300000000)
	{
		...
		// Enable VHF filter
		ret = fc0013_enable_vhf_filter(dev, 1);
		if (ret)
			goto exit;
		...
		*/
		...
		// Disable VHF filter
		ret = fc0013_enable_vhf_filter(dev, 0);
		if (ret)
			goto exit;
