...
// do we want the fewest ranges (easy) or the fewest bins (harder)?
{
	...
	for (i=0; i<tune_count; i++) {
		...
		printf("\tcrop: %f\n", ts->crop);
		printf("\tdownsample: %d\n", ts->downsample);
		printf("\tdownsample_passes: %d\n", ts->downsample_passes);
		// printf("\tavg: %ld\n", ts->avg);
		printf("\tbuf_len: %d\n", ts->buf_len);
...
void frequency_dvbt_range(int firstChannel, int lastChannel, double crop)
{
	int i, j, upper, lower, max_size, bw_seen, bw_used, bin_e, buf_len;
	...
	printf("Using DVB-T channels\n");

	downsample = 1;
	downsample_passes = 0;

	// Number of channels
	tune_count = lastChannel - firstChannel;
	...
	for (i=0; i<tune_count; i++)
	{
		int channel = i+firstChannel;

		int freq = (channel * 8 + 302) * 1000000;
...
//Dongle bandwidth: 1048575Hz
//Downsampling by: 1x
//Cropping by: 0.00%
//Total FFT bins: 1
//Logged FFT bins: 1
//FFT bin size: 0.00Hz
//Buffer size: 16384 bytes (7.81ms)
...
int main(int argc, char **argv)
{
	...
	// Parse frequency range argument
	if (f_set==1)
		frequency_range(freq_optarg, crop);
	else
		frequency_dvbt_range(21, 69, crop);
	...
	while (!do_exit)
	{
		// Obtain a single scan
		scanner();
		passes++;

		// Check if we have completed a single pass
		time_now = time(NULL);
