...
void usage(void)
{
	fprintf(stderr,
		...
		"\n"
		"Added by ome Niels:\n"
		"\t-n instead of -f/ will automatically scan all available DVB-T channels\n"
		"\t\t(UHF channels 21-69)\n"
		"\t-N instead of -f/-n will scan half of the channels (skip every other freq)\n"
		"\n"
...
/**
 * Configure scanning for DVB-T channels
 */
void frequency_dvbt_range(int firstChannel, int lastChannel, int skip, double crop)
{
	struct tuning_state *ts;
	...
	printf("Using DVB-T channels %d-%d\n", firstChannel, lastChannel);

	// Number of channels
	tune_count = 0;

	/* build the array */
	for (i=firstChannel; i<lastChannel; i+=skip)
	{
		int freq = (i * 8 + 302) * 1000000;

		ts = &tunes[tune_count++];

		// Left edge of the DVB-T channel
		ts->freq = freq;

		// 1 MHz sampling rate
		ts->rate = 1000000;

		// 256-sample FFT (2^8=256)
		ts->bin_e = bin_e;
		ts->samples = 0;
		ts->crop = crop;
		ts->downsample = downsample;
...
//		printf("%d samples read, performing FFT\n", n_read);
//		fflush(stdout);
	...
	long stopTime = stop.tv_sec * 1000 + stop.tv_usec / 1000;
	printf("Scan pass took %ld\n", stopTime - startTime);
}
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "f:i:s:t:d:g:p:e:w:c:F:1PDOhnN")) != -1) {
		switch (opt) {
	...
	if (f_set==1)
		...
		frequency_dvbt_range(21, 69, 1, crop);
	else if (f_set==3)
		frequency_dvbt_range(21, 69, 2, crop);
